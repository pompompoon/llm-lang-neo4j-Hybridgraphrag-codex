...
ブラウザで `http://localhost:1000/` を開く。`npm` 不要 — FastAPI が単一 HTML を配信する。
...
チャット回答ごとにルールベースの精度指標を算出し、LLM が使える場合は LLM 判定も併記する。

| 指標 | 内容 |
|---|---|
| faithfulness | 忠実性: 回答の各文が検索コンテキストに根拠を持つ割合 |
| answer_relevancy | 回答関連性: 質問の内容語を回答がカバーする割合 |
| context_relevancy | コンテキスト関連性: 検索ヒットのうち質問に関係する割合 |
| grounded_numbers | 数値根拠率: 回答中の数値がコンテキストに存在する割合 |
| overall | 上記の加重平均。品質ゲート（リトライ判定）に使用 |
| llm_* | LLM-as-judge による忠実性/関連性/完全性（PROVIDER が mock 以外のとき） |

- `/api/chat` のレスポンスに `metrics` が含まれ、チャットUIの回答下にも表示される
- 一括評価: `POST /api/evaluate` に `{"items": [{"question": "...", "expected": "任意の期待回答"}]}` を送ると、各質問の指標と平均値（`expected` 指定時は期待回答との内容語F1 `expected_match` も）を返す
